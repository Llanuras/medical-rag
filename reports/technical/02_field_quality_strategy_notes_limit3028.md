...
当前 `3028` 篇中 abstract 缺失率为 `8.3554%`。因此建议：分层处理：body 替代、title-only metadata、无文本丢弃。
...
- pmid 可用率：`90.9181%`，可用于 PubMed 追溯。
- pmcid 可用率：`100.0000%`，可用于 PMC 追溯。

## 7. 当前 `3028` 篇数据的实际结论

详见 `artifacts/metrics/t002_corpus_analysis/missing_rate_limit3028.csv`、`artifacts/metrics/t002_corpus_analysis/quality_summary_limit3028.csv` 和 `artifacts/metrics/t002_corpus_analysis/metadata_summary_limit3028.csv`。所有数值均来自真实解析的 `3028` 篇 XML。
